...
<script type="text/javascript">
				$(document).ready(function () {	
					...
					var txt = document.getElementById('textkol').value;
					...
					$("#textkol").change(function() {
						txt = document.getElementById('textkol').value;
						txt = txt.replace(/\s+/gi, "");
						cnt = txt.length;
						$("#textkol").empty();
						$("#textkol").append(cnt);
						$("p.textkol2").empty();
						$("p.textkol2").append(cnt);
					});					
				});
			</script>
<label>Размер букв</label>
<p>15x15 см.</p>
<input type=hidden name=raz value=15x15>
<p class="option-infos">* Цена указана за одну букву стандартного размера. Если Вы хотите изменить размер, то укажите это в комментарии к заказу.</p>			
<br>
<label>Ваш текст: </label>
<input type="text" name="text" id="textkol">
<br>
<label>Букв: </label>
<p class="textkol2">0</p>
<input type="hidden" name="text_price" value="50">
<h4>Цена за 1 букву 50 руб.</h4>
</div>
<input type="hidden" name="texton" value="texton">
<input type="hidden" name="textid" value=<?=$namefoto?>>
...
<a href="http://www.samokleyka.net/albom14/<?=$back?>.php">
...
<a href="http://www.samokleyka.net/albom14.php">
