...
<?php if ($comments) : ?>
<h3 id="comments"><?php comments_number('нет комментариев', '1 коммент.', '% коммент.' );?> на &#8220;<?php the_title(); ?>&#8221;</h3>
...
<p class="comment_author"><?php comment_author_link() ?> пишет:</p>
<p class="comment_time"><?php comment_date('F jS, Y') ?> в <?php comment_time() ?> <?php edit_comment_link('edit','&nbsp;&nbsp;',''); ?></p>
<div class="comment_text"><?php comment_text() ?></div>
<?php if ($comment->comment_approved == '0') : ?>
<br /><em>На модерации.</em>
<?php endif; ?>
</div>
<div class="clear"></div>
...
<h3 id="respond">Оставить сообщение</h3>

<?php if ( get_option('comment_registration') && !$user_ID ) : ?>

<p>Вы должны  <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=<?php echo urlencode(get_permalink()); ?>">зарегистрироваться</a> .</p>
...
<label for="author">Имя <?php if ($req) echo "(*)"; ?></label></p>
...
<label for="email">Почта  <?php if ($req) echo "(*)"; ?></label></p>
...
<p><input name="submit" type="submit" id="submit" tabindex="5" value="отправить" class="replybutton" />
