...
if(typeof rsv2=='undefined') rsv2={};

// SETTINGS
rsv2.displayType="alert-all";
rsv2.errorFieldClass=null;
rsv2.errorTextIntro="Please fix the following error(s) and resubmit:";
rsv2.errorJSItemBullet="* ";
rsv2.errorHTMLItemBullet="&bull; ";
rsv2.errorTargetElementId="rsv2Errors";
rsv2.customErrorHandler=null;
rsv2.onCompleteHandler=null;

// ~~~~~~~~~~~~~~~~~~~~~~~

rsv2.validate=function(form,rules)
{rsv2.returnHash=[];for(var i=0;i<rules.length;i++)
{var row=rules[i].replace(/\\,/ig,"%%C%%");row=row.split(",");var satisfiesIfConditions=true;while(row[0].match("^if:"))
{var cond=row[0];cond=cond.replace("if:","");var comparison="equal";var parts=[];if(cond.search("!=")!=-1)
{parts=cond.split("!=");comparison="not_equal";}
else
...
if(!satisfiesIfConditions)
continue;var requirement=row[0];var fieldName=row[1];var fieldName2,fieldName3,errorMessage,lengthRequirements,date_flag;if(requirement!="function"&&form[fieldName]==undefined)
{alert("rsv2 Error: the field \""+fieldName+"\" doesn't exist! Please check your form and settings.");return false;}
if(requirement!="function"&&rsv2.errorFieldClass)
{if(form[fieldName].type==undefined)
{for(var j=0;j<form[fieldName].length;j++)
rsv2.removeClassName(form[fieldName][j],rsv2.errorFieldClass,true);}
else
rsv2.removeClassName(form[fieldName],rsv2.errorFieldClass);}
if(row.length==6)
{fieldName2=row[2];fieldName3=row[3];date_flag=row[4];errorMessage=row[5];}
else if(row.length==5)
{fieldName2=row[2];fieldName3=row[3];errorMessage=row[4];}
...
oneIsChecked=true;}
if(!oneIsChecked)
{if(!rsv2.processError(form[fieldName],errorMessage))
return false;}}
else if(form[fieldName].type=="select-multiple")
{var oneIsSelected=false;for(var k=0;k<form[fieldName].length;k++)
{if(form[fieldName][k].selected)
...
return false;}}
break;}
break;case"valid_email":if(form[fieldName].value&&!rsv2.isValidEmail(form[fieldName].value))
{if(!rsv2.processError(form[fieldName],errorMessage))
return false;}
break;case"valid_date":var isLaterDate=false;if(date_flag=="later_date")
isLaterDate=true;else if(date_flag=="any_date")
isLaterDate=false;if(!rsv2.isValidDate(form[fieldName].value,form[fieldName2].value,form[fieldName3].value,isLaterDate))
{if(!rsv2.processError(form[fieldName],errorMessage))
return false;}
break;case"same_as":if(form[fieldName].value!=form[fieldName2].value)
{if(!rsv2.processError(form[fieldName],errorMessage))
...
break;case"function":custom_function=fieldName;eval("var result = "+custom_function+"()");if(result.constructor.toString().indexOf("Array")!=-1)
{for(var j=0;j<result.length;j++)
{if(!rsv2.processError(result[j][0],result[j][1]))
return false;}}
break;default:alert("Unknown requirement flag in validateFields(): "+requirement);return false;}}
if(typeof rsv2.customErrorHandler=='function')
return rsv2.customErrorHandler(form,rsv2.returnHash);else if(rsv2.displayType=="alert-all")
{var errorStr=rsv2.errorTextIntro+"\n\n";for(var i=0;i<rsv2.returnHash.length;i++)
{errorStr+=rsv2.errorJSItemBullet+rsv2.returnHash[i][1]+"\n";rsv2.styleField(rsv2.returnHash[i][0],i==0);}
if(rsv2.returnHash.length>0)
{alert(errorStr);return false;}}
else if(rsv2.displayType=="display-html")
{var success=rsv2.displayHTMLErrors(form,rsv2.returnHash);if(!success)
return false;}
if(typeof rsv2.onCompleteHandler=='function')
return rsv2.onCompleteHandler();else
return true;}
rsv2.processError=function(obj,message)
{message=message.replace(/%%C%%/ig,",");var continueProcessing=true;switch(rsv2.displayType)
{case"alert-one":alert(message);rsv2.styleField(obj,true);continueProcessing=false;break;case"alert-all":case"display-html":rsv2.returnHash.push([obj,message]);break;}
return continueProcessing;}
rsv2.displayHTMLErrors=function(f,errorInfo)
{var errorHTML=rsv2.errorTextIntro+"<br /><br />";for(var i=0;i<errorInfo.length;i++)
{errorHTML+=rsv2.errorHTMLItemBullet+errorInfo[i][1]+"<br />";rsv2.styleField(errorInfo[i][0],i==0);}
if(errorInfo.length>0)
{document.getElementById(rsv2.errorTargetElementId).style.display="block";document.getElementById(rsv2.errorTargetElementId).innerHTML=errorHTML;return false;}
return true;}
rsv2.styleField=function(field,focus)
{if(field.type==undefined)
{if(focus)
field[0].focus();for(var i=0;i<field.length;i++)
rsv2.addClassName(field[i],rsv2.errorFieldClass,true);}
else
{if(rsv2.errorFieldClass)
rsv2.addClassName(field,rsv2.errorFieldClass,true);if(focus)
field.focus();}}
rsv2.isValidEmail=function(str)
{var str2=str.replace(/^\s*/,"");var s=str2.replace(/\s*$/,"");var at="@";var dot=".";var lat=s.indexOf(at);var lstr=s.length;var ldot=s.indexOf(dot);if(s.indexOf(at)==-1||(s.indexOf(at)==-1||s.indexOf(at)==0||s.indexOf(at)==lstr)||(s.indexOf(dot)==-1||s.indexOf(dot)==0||s.indexOf(dot)==lstr)||(s.indexOf(at,(lat+1))!=-1)||(s.substring(lat-1,lat)==dot||s.substring(lat+1,lat+2)==dot)||(s.indexOf(dot,(lat+2))==-1)||(s.indexOf(" ")!=-1))
{return false;}
return true;}
rsv2.isWhitespace=function(s)
{var whitespaceChars=" \t\n\r\f";if((s==null)||(s.length==0))
return true;for(var i=0;i<s.length;i++)
{var c=s.charAt(i);if(whitespaceChars.indexOf(c)==-1)
return false;}
return true;}
rsv2.isValidDate=function(month,day,year,isLaterDate)
{var daysInMonth;if((year%4==0)&&((year%100!=0)||(year%400==0)))
daysInMonth=[31,29,31,30,31,30,31,31,30,31,30,31];else
daysInMonth=[31,28,31,30,31,30,31,31,30,31,30,31];if(!month||!day||!year)return false;if(1>month||month>12)return false;if(year<0)return false;if(1>day||day>daysInMonth[month-1])return false;if(isLaterDate)
{var today=new Date();var currMonth=today.getMonth()+1;var currDay=today.getDate();var currYear=today.getFullYear();if(String(currMonth).length==1)currMonth="0"+currMonth;if(String(currDay).length==1)currDay="0"+currDay;var currDate=String(currYear)+String(currMonth)+String(currDay);if(String(month).length==1)month="0"+month;if(String(day).length==1)day="0"+day;incomingDate=String(year)+String(month)+String(day);if(Number(currDate)>Number(incomingDate))
return false;}
return true;}
rsv2.addClassName=function(objElement,strClass,mayAlreadyExist)
{if(objElement.className)
{var arrList=objElement.className.split(' ');if(mayAlreadyExist)
{var strClassUpper=strClass.toUpperCase();for(var i=0;i<arrList.length;i++)
{if(arrList[i].toUpperCase()==strClassUpper)
...
objElement.className=strClass;}
rsv2.removeClassName=function(objElement,strClass)
{if(objElement.className)
{var arrList=objElement.className.split(' ');var strClassUpper=strClass.toUpperCase();for(var i=0;i<arrList.length;i++)
{if(arrList[i].toUpperCase()==strClassUpper)
{arrList.splice(i,1);i--;}}
